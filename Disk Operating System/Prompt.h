...
class prompt {
    ...
    directory* loadDirectory(ifstream& Rdr, directory* parent = nullptr) {
        string line;
        getline(Rdr, line);
        ...
        while (getline(Rdr, line)) {
            if (line[0] == '(') {
                directory* subDir = loadDirectory(Rdr, newDir);
                newDir->dir.push_back(subDir);
            }
            else if (line[0] == '+') {
                string fileName = line.substr(1);
                file* newFile = new file(fileName, newDir);
                newDir->f.push_back(newFile);
            }
        }
